...
//        Fourth: Open a browser and open url
        if(result_message.indexOf("qual") != -1 || result_message.indexOf("que") != -1 ){
            if(result_message.indexOf("seu nome") != -1){
                speak("Meu Nome é Liza.Prazer em conhecer você!");
            }
            if (result_message.indexOf("horas") != -1){
                String time_now = DateUtils.formatDateTime(this, new Date().getTime(),DateUtils.FORMAT_SHOW_TIME);
                speak("Agora são: " + time_now);
            }
        } else if (result_message.indexOf("terra") != -1){
            speak("Não seja bobo, a terra é uma esfera. Assim como todos os outros planetas e corpos celestes");
        } else if (result_message.indexOf("navegador") != -1){
        ...
        tts = new TextToSpeech(this, new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if (tts.getEngines().size() == 0 ){
                    ...
                    speak("Olá Douglas, Eu estou pronta para começar a conversar");
                }
            }
        });
    }
